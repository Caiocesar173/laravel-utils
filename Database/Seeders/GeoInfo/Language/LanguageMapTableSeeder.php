...
class LanguageMapTableSeeder extends Seeder
{
    /**
    ...
    public function run() {
        ...
        DB::table('language_maps')->truncate();
        $counter = 0;
        foreach ($languages as $languageMap) 
        {
            ...
            $save = LanguageMap::create($languageMap);
            if (is_string($save))
                throw new \Exception($save, 1);

            if($this->command)
                $this->command->info("Inserted language {$language["name"]} from country $country->code [$counter].");
            else
                echo("Inserted language {$language["name"]} from country $country->code [$counter].\n");
