...
class PhoneAreaTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($phoneAreas as $phone) {
            ...
            $save = PhoneArea::create($phone);
            if (is_string($save))
                throw new \Exception($save, 1);

            if ($this->command)
                $this->command->info("Inserted phone with code {$phone['code']} from country $country->name [$counter].");
            else
                echo ("Inserted phone with code {$phone['code']} from country $country->name [$counter].");
