...
namespace Caiocesar173\Utils\Database\Seeders\GeoInfo\Zones;
...
class TimeZoneMapTableSeeder extends Seeder
{
    /**
    ...
    public function run() {
        ...
        DB::table('time_zone_maps')->truncate();
        $counter = 0;
        foreach ($timeZones as $timeZone) 
        {
            ...
            $save = TimeZoneMap::create($timeZone);
            if (is_string($save))
                throw new \Exception($save, 1);

            if($this->command)
                $this->command->info("Inserted time zone {$timeZone['zone']} from $country->code [$counter].");
            else
                echo("Inserted time zone {$timeZone["zone"]} from $country->code [$counter].\n");
